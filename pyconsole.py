...
class Console:
	...
	def submit_input(self, text,glob):
		'''\
		Submit input
	...
	def send_pyconsole(self, text, glob):
		'''\
		...
		for event in eventlist:
			if event.type == KEYDOWN:
				...
				if event.key == K_TAB:
					...
					self.submit_input(self.c_in,glob)
